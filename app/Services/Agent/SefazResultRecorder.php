...
class SefazResultRecorder
{
    public function recordSuccessfulResult(AgentCommand $command, CommandResultData $data): void
    {
        ...
        $request = SefazRequest::query()->create([
            ...
            'service' => $data->sefaz['service'] ?? $data->result['service'] ?? $this->serviceFromCommand($command),
            'environment' => $data->sefaz['environment'] ?? $data->result['environment'] ?? FiscalEnvironment::Production->value,
            'endpoint' => $data->sefaz['endpoint'] ?? $data->result['endpoint'] ?? 'agent-local',
            'soap_action' => $data->sefaz['soap_action'] ?? $data->result['soap_action'] ?? null,
            ...
            'correlation_id' => $data->sefaz['correlation_id'] ?? $data->result['correlation_id'] ?? $command->uuid,
